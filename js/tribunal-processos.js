...
function tfObterUtilizadorLocal() {
  try { return JSON.parse(localStorage.getItem(TF_CHAVE_UTILIZADOR) || "null"); }
...
function tfRenderizarProcessos(processos) {
  ...
  if (!processos.length) {
    lista.innerHTML = `<div class="tribunal-vazio"><strong>Nenhum processo encontrado.</strong><p>Altere os filtros ou registe um novo processo simulado.</p></div>`;
    return;
  }
  lista.innerHTML = processos.map(function (processo) {
    const id = encodeURIComponent(processo.ID_Processo || "");
    return `
      <article class="tribunal-processo-item">
        <div class="tribunal-processo-identificacao">
          ...
          <div class="tribunal-processo-acoes">
            ...
            <a class="tribunal-acao-link" href="tribunal-decisoes-avaliacao.html?idProcesso=${id}">Decisões e avaliação</a>
          </div>
        </div>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function carregarProcessos() {
    ...
    try {
      const resultado = await tfChamarApi({
        acao: "tfListarProcessos", token: token,
      ...
      tfRenderizarProcessos([]);
      if (erro.message !== "Sessão expirada.") tfMensagem(mensagemLista, "Não foi possível carregar os processos. Verifique a ligação e tente novamente.", "erro");
    }
  }
  ...
  btnAbrir.addEventListener("click", function () {
    ...
    tfAbrirModal(modal);
    setTimeout(function () { document.getElementById("tipoProcessoTF").focus(); }, 50);
  });

  [btnFechar, btnCancelar].forEach(function (botao) {
    botao.addEventListener("click", function () { tfFecharModal(modal); });
  });

  modal.addEventListener("click", function (evento) {
    if (evento.target.dataset.fecharModal === "true") tfFecharModal(modal);
  });

  document.addEventListener("keydown", function (evento) {
    if (evento.key === "Escape" && !modal.classList.contains("oculto")) tfFecharModal(modal);
  });
  ...
  form.addEventListener("submit", async function (evento) {
    ...
    try {
      const resultado = await tfChamarApi({
        acao: "tfCriarProcesso", token: token,
      ...
      if (!resultado.sucesso) { tfMensagem(mensagemForm, resultado.mensagem || "Não foi possível criar o processo.", "erro"); return; }
      tfMensagem(mensagemForm, `Processo ${resultado.numeroProcesso || resultado.idProcesso} criado com sucesso.`, "sucesso");
      form.reset();
      await carregarProcessos();
      setTimeout(function () { tfFecharModal(modal); }, 900);
    } catch (erro) {
      if (erro.message !== "Sessão expirada.") tfMensagem(mensagemForm, "Não foi possível criar o processo. Tente novamente.", "erro");
    } finally {
  ...
  document.getElementById("tfPesquisa").addEventListener("keydown", function (evento) { if (evento.key === "Enter") carregarProcessos(); });
